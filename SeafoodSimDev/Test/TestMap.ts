class TestMap {
    public constructor() {
        ...
        var map: Map = new Map(new Restrictions(scenario), scenario);

        QUnit.test("Map: add ship", function (assert) {
            var ship: Ship = new Ship(new ShipOwner(gov, new Point2(0, 0),"0", scenario), scenario);
        ...
        QUnit.test("Map: fish", function (assert) {
            ...
            var codScool: Cod = new Cod(noOfFish, 80, fishingPoint, scenario);
        ...
        QUnit.test("Map: fish with full capacity", function (assert) {
            ...
            var codScool: Cod = new Cod(noOfFish, 80, fishingPoint, scenario)
            map.addSchool(codScool);
            var capacity: number = noOfFish * scenario.getFishingPercentage() / 2;
        ...
        QUnit.test("Map: get number of ships in tile", function (assert) {
            map = new Map(gov.getRestrictions(), scenario);
            var owner: ShipOwner = new ShipOwner(gov, new Point2(4, 4), "0", scenario);
