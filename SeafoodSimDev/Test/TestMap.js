var TestMap = (function () {
    function TestMap() {
        var map = new Map(5, 0, new Restrictions());
        QUnit.test("Map: add ship", function (assert) {
            var ship = new Ship(new ShipOwner(new Point2(0, 0), "0"));
        ...
        QUnit.test("Map: remove ship", function (assert) {
            var ship = new Ship(new ShipOwner(new Point2(0, 0), "1"));
            map.addShip(ship);
            assert.ok(map.getShips().indexOf(ship) > -1, "ship should be in map");
            map.removeShip(ship);
            assert.deepEqual(map.getShips().indexOf(ship), -1, "ship should not be in map");
        ...
        QUnit.test("Map: fish", function (assert) {
            ...
            var noOfFish = 1000;
            var codScool = new Cod(noOfFish, fishingPoint);
            map.addSchool(codScool);
            var fish = map.fish(fishingPoint, 100);
        ...
        QUnit.test("Map: fish with full capacity", function (assert) {
            ...
            var codScool = new Cod(noOfFish, fishingPoint);
            map.addSchool(codScool);
            var capacity = noOfFish * map.getFishingPercentage() / 2;
            var fish = map.fish(fishingPoint, capacity);
        ...
        QUnit.test("Map: get number of ships in tile", function (assert) {
            map = new Map(5, 0, new Restrictions());
            var owner = new ShipOwner(new Point2(4, 4), "0");
