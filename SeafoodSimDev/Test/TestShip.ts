...
class TestShip {
    ...
    public runTests = (): void => {
        ...
        QUnit.test("Ship: fish under restrictions", function (assert) {
            map.emptyGrid(100);
            var codShip: Ship = new Ship(owner, FishType.cod, new Point2(0, 1));
            var cod: Cod = new Cod(10000, new Point2(0, 0))
            map.addSchool(cod);
            ...
            var cargo: number[] = codShip.getCargo()[FishType.cod];
            var oldThreshhold: number = Math.ceil(0.3 * cod.getMaxAge());
